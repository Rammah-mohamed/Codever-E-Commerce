...
const Cart = () => {
	...
	const cartValue = () => {
		...
		if (value.length !== 0) {
			if (value.length === 1) {
				sum = value.map((p) => [p.quantity, p.price]).reduce((acc, curr) => curr[0] * curr[1], 0);
			} else {
				sum = value.map((p) => p.quantity * p.price).reduce((acc, curr) => acc + curr);
			}
	...
	const handleClick = () => {
		...
		if (value.length !== 0) {
			if (value.length === 1) {
				ordersQuantity = value.map((p) => p.quantity).reduce((acc, curr) => acc + curr, 0);
			} else {
				ordersQuantity = value.map((p) => p.quantity).reduce((acc, curr) => acc + curr);
			}
		}
